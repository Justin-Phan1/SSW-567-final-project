class MRTD:
    ...
    def encode_mrz_data(record):
        ...
        cd1 = MRTD.adler_32(passport_number) % 10
        
        country_code = line2_data["country_code"]
        birth_date = line2_data["birth_date"]
        cd2 = MRTD.adler_32(birth_date) % 10 
        
        sex = line2_data["sex"]
        expiration_date = line2_data["expiration_date"]
        cd3 = MRTD.adler_32(expiration_date) % 10
        
        personal_number = line2_data["personal_number"].ljust(15, '<')
        cd4 = MRTD.adler_32(personal_number) % 10
        
        # Calculate final check digit (CD5) over specific fields
        final_check_data = passport_number + str(cd1) + birth_date + str(cd2) + expiration_date + str(cd3) + personal_number + str(cd4)
        cd5 = MRTD.adler_32(final_check_data) % 10
        
        # Line 2 construction
        line2 = (passport_number + str(cd1) + country_code + birth_date + str(cd2) + 
                sex + expiration_date + str(cd3) + personal_number + str(cd4))
                # "<<<<<<" + cd5)
